...
const getRequestRole = (req) => req.headers['x-user-role'] || req.userRole || null;
...
const getCourseDetails = async (req, res, next) => {
  try {
    ...
    const assignmentsPromise = Assignment.find({ course: courseId })
      .sort({ dueDate: 1 })
      .lean();
...
const enrollStudent = async (req, res, next) => {
  try {
    ...
    if (course.students.some((student) => student.toString() === studentId)) {
      return res
        .status(409)
        .json({ success: false, message: 'Student already enrolled in this course' });
    }

    if (course.enrolled >= course.capacity) {
      return res
        .status(400)
        .json({ success: false, message: 'Course capacity has been reached' });
    }
